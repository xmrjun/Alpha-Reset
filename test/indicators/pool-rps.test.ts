...
test('RPS 按完整观察池排名，不接收 A1/A2 过滤参数', () => {
  ...
  const scores = calculatePoolRps(members, now, loadStrategy());
...
test('小时线可补精确端点，不能拿邻近小时近似半小时端点', () => {
  ...
  assert.equal(calculatePoolRps([value], now, loadStrategy()).get('a')!.r288, 0);
  value.candles15m = [candle(now + HOUR_MS / 4, 110)];
  assert.equal(calculatePoolRps([value], now + HOUR_MS / 2, loadStrategy()).get('a')!.r288, null);
});
...
test('配置变更窗口、忽略未收盘与陈旧当前价格，输入不变', () => {
  ...
  assert.equal(calculatePoolRps([value], now + INTERVAL_MS['15m'], loadStrategy()).get('a')!.r16, null);
});
